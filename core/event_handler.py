    ...
    async def handle_recall(self, user_id: int, msg_id: int, content: str,
        ...
        if unknown:
            ...
            recall_info = (
                ...
                "你不知道用户撤回了什么具体内容。\n"
                "\n"
                "默认情况下你应该简单回应一下 (比如 \"嗯?撤回了啥?\" \"诶?\" 等), \n"
                "让用户感觉你在关注ta。只有在你觉得确实没话可说时才输出 [SKIP_RECALL] 不回应。\n"
                "\n"
                "绝对不能编造/猜测撤回的具体内容。"
            )
        else:
            ...
            recall_info = (
                ...
                f'\n'
                f'请结合你的人设、当前对话上下文、以及撤回的内容, 决定如何回应。\n'
                f'默认情况下你应当主动回应 (让用户感觉你在关注ta), 除非确实没有合适的话可说。\n'
                f'注意: 你的回复要让用户明确感觉到你注意到了"撤回"这件事,\n'
                f'可以自然地点明 (如 "你刚撤回了? 我看到了" / "诶? 你撤回的那句是..." 等),\n'
                f'不要表现得像只是收到一条普通消息那样继续聊。\n'
                f'\n'
                f'参考方向 (不限于此, 由你的人设主宰):\n'
                ...
                f'- 温柔包容: "没关系的, 不想说就不说"\n'
                f'\n'
                f'决定: 直接输出回复文本 (不要带前缀标记)。\n'
                f'只有当你确实觉得不该回应时, 才输出 [SKIP_RECALL]。\n'
